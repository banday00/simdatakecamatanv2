...
export async function PATCH(req: NextRequest, context: RouteContext) {
    try {
        ...
        return ok(await updateAdminKetentramanResource(tenant, "insiden", uuidSchema.parse(id), req));
    } catch (error) {
...
export async function DELETE(req: NextRequest, context: RouteContext) {
    try {
        const { tenant, id } = await context.params;
        return ok(await deleteAdminKetentramanResource(tenant, "insiden", uuidSchema.parse(id), req));
    } catch (error) {
        return handleApiError(error);
    }
}
