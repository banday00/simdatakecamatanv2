...
export default function KaderPage() {
    const { data, isLoading, error, kelurahanOptions, kelMap, createRow, updateRow, deleteRow: deleteData } =
        useAdminKetentramanResource<Row>("kader", "data pengawasan kader");
    ...
    const totalLinmas = data.reduce((s, r) => s + Number(r.jumlah_linmas || 0), 0);
    const totalSatgas = data.reduce((s, r) => s + Number(r.jumlah_satgas || 0), 0);
    const totalPos = data.reduce((s, r) => s + Number(r.pos_kamling || 0), 0);
    const totalSiskamling = data.reduce((s, r) => s + Number(r.kegiatan_siskamling || 0), 0);
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch (err) { alert(err instanceof Error ? err.message : "Gagal menyimpan data pengawasan kader."); } finally { setIsSubmitting(false); }
    }
