...
export default function DisabilitasPage() {
    const { data, isLoading, error, kelurahanOptions, kelMap, createRow, updateRow, deleteRow: deleteData } =
        useAdminSosialResource<Row>("disabilitas", "data disabilitas");
    ...
    const totalDis = data.reduce((s, r) => s + Number(r.jumlah || 0), 0);
    const totalBantuan = data.reduce((s, r) => s + Number(r.penerima_bantuan || 0), 0);
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch (err) { alert(err instanceof Error ? err.message : "Gagal menyimpan data disabilitas."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await deleteData(deleteRow.id); setDeleteRow(null); }
        catch (err) { alert(err instanceof Error ? err.message : "Gagal menghapus data disabilitas."); } finally { setIsSubmitting(false); }
    }
