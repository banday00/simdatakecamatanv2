...
export default function BantuanSosialPage() {
    const { data, isLoading, error, kelurahanOptions, kelMap, createRow, updateRow, deleteRow: deleteData } =
        useAdminSosialResource<Row>("bantuan", "data bantuan sosial");
    ...
    const totalPenerima = data.reduce((s, r) => s + Number(r.jumlah_penerima || 0), 0);
    const totalAnggaran = data.reduce((s, r) => s + Number(r.total_anggaran || 0), 0);
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch (err) { alert(err instanceof Error ? err.message : "Gagal menyimpan data bantuan sosial."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await deleteData(deleteRow.id); setDeleteRow(null); }
        catch (err) { alert(err instanceof Error ? err.message : "Gagal menghapus data bantuan sosial."); } finally { setIsSubmitting(false); }
    }
