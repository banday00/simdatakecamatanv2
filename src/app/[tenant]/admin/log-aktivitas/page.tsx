...
export default function LogAktivitasPage() {
    ...
    const fetchLogs = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/activity-logs?${params.toString()}`, {
                cache: "no-store",
            });
            const result = await response.json();
            if (!response.ok || result.error || !result.data) {
                throw new Error(result.error?.message ?? "Gagal memuat log aktivitas.");
            }

            setData((result.data.rows as LogRow[]) || []);
            setTotalCount(result.data.totalCount || 0);
            setStats({
                total: result.data.stats?.total || 0,
                login: result.data.stats?.login || 0,
                crud: result.data.stats?.crud || 0,
                uniqueUsers: result.data.stats?.unique_users || 0,
            });
        } catch (err) {
    ...
    }, [tenant?.slug, page, filterAction, filterSearch, filterDateFrom, filterDateTo]);
