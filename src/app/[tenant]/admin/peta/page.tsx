...
export default function PetaAdminPage() {
    ...
    const [poi, setPoi] = useState<PoiRow[]>([]);
    ...
    const refreshData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/peta`, { cache: "no-store" });
            const result = (await response.json()) as PetaAdminResponse;
            if (!response.ok || result.error || !result.data) {
                throw new Error(result.error?.message ?? "Gagal memuat data peta.");
            }

            setLayers(result.data.layers);
            setPoi(result.data.poi);
            setPreviewFacilities(result.data.facilities);
        } catch (error) {
    ...
    async function sendMutation<T>(path: string, options: RequestInit) {
        ...
        const result = (await response.json()) as ApiMutationResponse<T>;
        if (!response.ok || result.error || !result.data) {
            throw new Error(result.error?.message ?? "Operasi gagal.");
        }
        return result.data;
    }
    ...
    async function handleLayerSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            const path = editLayer
                ? `/api/tenants/${tenant.slug}/admin/peta/layers/${editLayer.id}`
                : `/api/tenants/${tenant.slug}/admin/peta/layers`;
            const row = await sendMutation<LayerRow>(path, {
                method: editLayer ? "PATCH" : "POST",
            ...
            setLayers((prev) => {
                ...
                return [row, ...prev].sort((a, b) => (a.urutan ?? 0) - (b.urutan ?? 0));
            });
    ...
    async function handlePoiSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            const path = editPoi
                ? `/api/tenants/${tenant.slug}/admin/peta/poi/${editPoi.id}`
                : `/api/tenants/${tenant.slug}/admin/peta/poi`;
            const row = await sendMutation<PoiRow>(path, {
                method: editPoi ? "PATCH" : "POST",
                body: JSON.stringify(payload),
            });
    ...
    const activeLayers = layers.filter((r) => r.is_visible).length;
    const totalPoi = poi.length;
    const poiKategori = new Set(poi.map((p) => p.kategori)).size;
    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <DeleteConfirm
                ...
                onConfirm={async () => {
                    if (!deleteLayer || !tenant) return;
                    setLayerSubmitting(true);
                    try {
                        await sendMutation<LayerRow>(`/api/tenants/${tenant.slug}/admin/peta/layers/${deleteLayer.id}`, {
                            method: "DELETE",
                        });
                        setLayers((prev) => prev.filter((item) => item.id !== deleteLayer.id));
                        setDeleteLayer(null);
                    }
            ...
            <DeleteConfirm
                ...
                onConfirm={async () => {
                    if (!deletePoi || !tenant) return;
                    setPoiSubmitting(true);
                    try {
                        await sendMutation<PoiRow>(`/api/tenants/${tenant.slug}/admin/peta/poi/${deletePoi.id}`, {
                            method: "DELETE",
                        });
                        setPoi((prev) => prev.filter((item) => item.id !== deletePoi.id));
                        await refreshData();
                        setDeletePoi(null);
                    }
                    catch { alert("Gagal menghapus"); }
