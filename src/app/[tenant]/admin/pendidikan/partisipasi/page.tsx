...
export default function PartisipasiPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/tenants/${tenant.slug}/admin/pendidikan/partisipasi`);
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal memuat partisipasi pendidikan");
            }
            setData(json.data || []);
        } catch (err) {
            const message = err instanceof Error ? err.message : "Gagal memuat partisipasi pendidikan";
            setError(message);
            console.error("[PendidikanPartisipasi] fetchData:", err);
        } finally {
            setIsLoading(false);
        }
    }, [tenant?.slug]);
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            const url = editRow
                ? `/api/tenants/${tenant.slug}/admin/pendidikan/partisipasi/${editRow.id}`
                : `/api/tenants/${tenant.slug}/admin/pendidikan/partisipasi`;
            ...
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal menyimpan partisipasi pendidikan");
            }
            await fetchData();
            setModalOpen(false);
            setEditRow(null);
    ...
    async function handleDelete() {
        ...
        setIsSubmitting(true);
        try {
            const res = await fetch(`/api/tenants/${tenant.slug}/admin/pendidikan/partisipasi/${deleteRow.id}`, {
                method: "DELETE",
            });
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal menghapus partisipasi pendidikan");
            }
            await fetchData();
            setDeleteRow(null);
        }
...
function PartisipasiFormModal({
    open, onClose, onSubmit, editRow, isSubmitting, kelurahanOptions, isKelurahanAdmin, filterKelurahanId, tenantSlug,
}: {
    open: boolean;
    onClose: () => void;
    ...
    const fetchSiswa = useCallback(async (kelurahanId: string, tahun: number, semester: number) => {
        ...
        try {
            ...
            const res = await fetch(`/api/tenants/${tenantSlug}/admin/pendidikan/partisipasi/siswa?${params.toString()}`, {
                signal: controller.signal,
            });
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal memuat data siswa");
            }

            if (controller.signal.aborted) return;

            setSiswaFromFacilities({
                sd: Number(json.data?.sd) || 0,
                smp: Number(json.data?.smp) || 0,
                sma: Number(json.data?.sma) || 0,
            });
        } catch (err: any) {
