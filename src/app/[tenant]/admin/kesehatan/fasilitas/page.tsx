...
export default function FasilitasKesehatanPage() {
    ...
    const [data, setData] = useState<FasilitasRow[]>([]);
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/tenants/${tenant.slug}/admin/kesehatan/fasilitas`);
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal memuat fasilitas kesehatan");
            }
            setData(json.data?.rows || []);
            setJenisRefs(json.data?.refs?.jenisFasilitas || []);
        } catch (err) {
            const message = err instanceof Error ? err.message : "Gagal memuat fasilitas kesehatan";
            setError(message);
            console.error("[Fasilitas] fetchData:", err);
        } finally {
            setIsLoading(false);
        }
    }, [tenant?.slug]);
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            const url = editRow
                ? `/api/tenants/${tenant.slug}/admin/kesehatan/fasilitas/${editRow.id}`
                : `/api/tenants/${tenant.slug}/admin/kesehatan/fasilitas`;
            ...
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal menyimpan fasilitas");
            }
            await fetchData();
            setModalOpen(false);
            setEditRow(null);
    ...
    async function handleDelete() {
        ...
        setIsSubmitting(true);
        try {
            const res = await fetch(`/api/tenants/${tenant.slug}/admin/kesehatan/fasilitas/${deleteRow.id}`, {
                method: "DELETE",
            });
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal menghapus fasilitas");
            }
            await fetchData();
            setDeleteRow(null);
        }
