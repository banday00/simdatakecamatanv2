...
export default function ProfilPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const res = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/profil`);
            const json = await res.json();
            if (!res.ok || json.error) {
                throw new Error(json.error?.message || "Gagal memuat profil pemerintahan");
            }

            setKecamatanData((json.data?.kecamatan as ProfilRow[]) || []);
            setKelurahanData((json.data?.kelurahan as ProfilRow[]) || []);
            setLeaders((json.data?.leaders as OrgLeader[]) || []);
        } catch (err) {
    ...
    async function handleCreate(formData: ProfilForm) {
        ...
        const json = await res.json();
        if (!res.ok || json.error) {
            throw new Error(json.error?.message || "Gagal menambah profil");
        }
    }
    ...
    async function handleUpdate(id: string, formData: ProfilForm) {
        ...
        const res = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/profil/${id}`, {
            method: "PATCH",
            headers: { "content-type": "application/json" },
            body: JSON.stringify(payload),
        });
        const json = await res.json();
        if (!res.ok || json.error) {
            throw new Error(json.error?.message || "Gagal memperbarui profil");
        }
    }
    ...
    async function handleDelete(id: string) {
        ...
        const json = await res.json();
        if (!res.ok || json.error) {
            throw new Error(json.error?.message || "Gagal menghapus profil");
        }
    }
    ...
    async function handleSubmit(formData: ProfilForm) {
        ...
        try {
            ...
            if (formData.peta_wilayah_file) {
                ...
                const fileName = `peta_${Date.now()}.${fileExt}`;
                const uploadForm = new FormData();
                uploadForm.set("path", `${tenant?.slug}/pemerintahan/profil/${fileName}`);
                ...
                finalPetaUrl = uploadJson.data.publicUrl;
            }
