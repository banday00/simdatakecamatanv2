...
import { type FieldDef } from "@/components/ui/form-modal";
...
type RefResponse = {
    periodes: { id: number; tahun: number; semester: number; keterangan: string | null }[];
    refs: Record<string, { id: number; label: string }[]>;
};
...
function TabPanel({
    ...
    const fetchData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/kependudukan/${config.key}`, { cache: "no-store" });
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal memuat data kependudukan."));
            }
            setData((result.data as Row[]) || []);
        } catch (err) {
            console.error(`[Kependudukan] fetchData unexpected error:`, err);
            alert(err instanceof Error ? err.message : "Gagal memuat data kependudukan.");
        } finally {
            setIsLoading(false);
        }
    }, [tenant?.slug, config.key]);
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            const url = editRow
                ? `/api/tenants/${tenant.slug}/admin/pemerintahan/kependudukan/${config.key}/${editRow.id}`
                : `/api/tenants/${tenant.slug}/admin/pemerintahan/kependudukan/${config.key}`;
            ...
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal menyimpan data kependudukan."));
            }
    ...
    async function handleDelete() {
        ...
        setIsSubmitting(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/kependudukan/${config.key}/${deleteRow.id}`, {
                method: "DELETE",
            });
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal menghapus data kependudukan."));
            }
            setDeleteRow(null);
            await fetchData();
        } catch (err: any) {
...
export default function KependudukanPage() {
    ...
    useEffect(() => {
        ...
        async function loadRefs() {
            try {
                const response = await fetch(`/api/tenants/${tenantSlug}/admin/pemerintahan/kependudukan/refs`, { cache: "no-store" });
                const result = await response.json();
                if (!response.ok || result.error) {
                    throw new Error(getApiErrorMessage(result, "Gagal memuat referensi kependudukan."));
                }

                const refs = result.data as RefResponse;
                setPeriodes(
                    (refs.periodes || []).map((p) => ({
                ...
                setRefData(
                    Object.fromEntries(
                        Object.entries(refs.refs || {}).map(([key, rows]) => [
                            key,
                            rows.map((row) => ({
                                label: row.label,
                                value: String(row.id),
                ...
                console.error("[Kependudukan] loadRefs error:", error);
                alert(error instanceof Error ? error.message : "Gagal memuat referensi kependudukan.");
            }
        }
