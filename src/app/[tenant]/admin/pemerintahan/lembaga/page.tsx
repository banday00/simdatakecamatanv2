...
export default function LembagaPage() {
    ...
    const [data, setData] = useState<LembagaRow[]>([]);
    ...
    const fetchData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/lembaga`, { cache: "no-store" });
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal memuat data lembaga."));
            }
            setData((result.data as LembagaRow[]) || []);
        } catch (error) {
            console.error("[Lembaga] fetchData error:", error);
            alert(error instanceof Error ? error.message : "Gagal memuat data lembaga.");
        } finally {
            setIsLoading(false);
        }
    }, [tenant?.slug]);
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            const response = await fetch(
                editRow
                    ? `/api/tenants/${tenant.slug}/admin/pemerintahan/lembaga/${editRow.id}`
                    : `/api/tenants/${tenant.slug}/admin/pemerintahan/lembaga`,
                {
            ...
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal menyimpan data lembaga."));
            }
    ...
    async function handleDelete() {
        ...
        setIsSubmitting(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/admin/pemerintahan/lembaga/${deleteRow.id}`, {
                method: "DELETE",
            });
            const result = await response.json();
            if (!response.ok || result.error) {
                throw new Error(getApiErrorMessage(result, "Gagal menghapus data lembaga."));
            }
            setDeleteRow(null);
            await fetchData();
        } catch (err: any) {
