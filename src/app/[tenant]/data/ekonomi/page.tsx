...
export default function EkonomiPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/data/ekonomi`, { cache: "no-store" });
            const result = await response.json();
            if (!response.ok || result.error || !result.data) {
                throw new Error(result.error?.message ?? "Gagal memuat data ekonomi.");
            }

            setSectors(result.data.sectors || []);
            setFacilities(result.data.facilities || []);
            setPotentials(result.data.potentials || []);
