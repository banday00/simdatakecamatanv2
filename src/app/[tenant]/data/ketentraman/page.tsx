...
export default function KetentaramanPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/tenants/${tenant.slug}/data/ketentraman`, { cache: "no-store" });
            const result = await response.json();
            if (!response.ok || result.error || !result.data) {
                throw new Error(result.error?.message ?? "Gagal memuat data ketentraman.");
            }
            setIncidentData(result.data.incidents || []);
            setDisasterData(result.data.disasters || []);
            setCadreData(result.data.cadres || []);
        } catch (err: any) {
