...
const KEPEMILIKAN_OPTIONS = ["Pemerintah Kota", "Pemerintah Provinsi", "Pemerintah Pusat", "Swasta", "BUMN/BUMD", "LSM/Yayasan", "Perorangan", "Lainnya"];
...
function useRefJenisSaranaOlahraga() {
    const [items, setItems] = useState<RefJenisSaranaOlahraga[]>([]);
...
function useInfraSports(tenantId: string | undefined) {
    ...
    const load = useCallback(async () => {
        ...
        const { data: rows } = await (supabase as any)
            .schema("sidakota")
            .from("infra_sports")
            .select("*, ref_jenis_sarana_olahraga:jenis_id(id, nama)")
        ...
        setData(
            (rows ?? []).map((r: any) => ({
                ...r,
                jenis_nama: r.ref_jenis_sarana_olahraga?.nama ?? "-",
            }))
        );
        setIsLoading(false);
    }, [tenantId]);
...
export default function OlahragaPage() {
    ...
    const { data: rawData, isLoading, reload } = useInfraSports(tenant?.id);
    const refJenis = useRefJenisSaranaOlahraga();
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        try {
            ...
            if (editRow) {
                await (supabase as any).schema("sidakota").from("infra_sports").update(payload).eq("id", editRow.id);
            } else {
                await (supabase as any).schema("sidakota").from("infra_sports").insert(payload);
            }
            setModalOpen(false);
            setEditRow(null);
            await reload();
    ...
    async function handleDelete() {
        ...
        try {
            const supabase = createClient();
            await (supabase as any).schema("sidakota").from("infra_sports").delete().eq("id", deleteRow.id);
            setDeleteRow(null);
            await reload();
        } catch { alert("Gagal menghapus data fasilitas olahraga."); }
        finally { setIsSubmitting(false); }
    }
