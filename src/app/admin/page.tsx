...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        async function loadStats() {
            setLoading(true);
            const [popRes, lemRes, fasRes, newsRes] = await Promise.all([
                supabase.schema("sidakota").from("gov_fact_populasi_summary").select("jml_penduduk_total, periode_id").eq("tenant_id", tenant!.id).order("periode_id", { ascending: false }),
                supabase.schema("sidakota").from("gov_institutions").select("id", { count: "exact" }).eq("tenant_id", tenant!.id),
                supabase.schema("sidakota").from("health_facilities").select("id", { count: "exact" }).eq("tenant_id", tenant!.id),
                supabase.schema("sidakota").from("news_articles").select("id", { count: "exact" }).eq("tenant_id", tenant!.id),
            ]);

            // Filter population data for the latest period to avoid summing multiple periods
            const latestPeriod = popRes.data?.[0]?.periode_id;
            const latestPopData = popRes.data?.filter(d => d.periode_id === latestPeriod) || [];
            setTotalPenduduk(latestPopData.reduce((s: number, r: Record<string, unknown>) => s + (Number(r.jml_penduduk_total) || 0), 0) ?? 0);

            setTotalLembaga(lemRes.count ?? 0);
            setTotalFasilitas(fasRes.count ?? 0);
            setTotalBerita(newsRes.count ?? 0);
            setLoading(false);
        }
        loadStats();
    }, [tenant?.id]);
