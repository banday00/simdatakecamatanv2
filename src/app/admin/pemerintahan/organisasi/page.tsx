        ...
        render: (val, row) => (
            <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0">
                {val ? (
                    ...
                    <img
                        src={String(val).startsWith('http') ? String(val) : `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/gov-profiles/${val}`}
                                        ...
                                        <div className="w-24 h-32 shrink-0 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center bg-gray-50 overflow-hidden relative group">
                                            {previewFoto ? (
                                                <>
                                                    {/* eslint-disable-next-line @next/next/no-img-element */}
                                                    <img
                                                        src={previewFoto.startsWith('http') || previewFoto.startsWith('blob:') || previewFoto.startsWith('data:') ? previewFoto : `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/gov-profiles/${previewFoto}`}
                                                        alt="Preview"
...
export default function OrganisasiPage() {
    ...
    async function handleSubmit(formData: OrgForm) {
        ...
        try {
            ...
            if (formData.foto_file && tenant?.id) {
                const file = formData.foto_file;
                const fileExt = file.name.split('.').pop();
                const fileName = `pejabat_${tenant.id}_${Date.now()}.${fileExt}`;
