...
export default function PerumahanPage() {
    ...
    const { data, isLoading, create, update, remove } = useCrud<Row>({ table: "social_rtlh" });
    ...
    const belumRehab = data.reduce((s, r) => s + (r.belum_direhabilitasi || 0), 0);
    const totalAnggaran = data.reduce((s, r) => s + (r.anggaran_rehabilitasi || 0), 0);
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch { alert("Gagal menyimpan data perumahan."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await remove(deleteRow.id); setDeleteRow(null); }
        catch { alert("Gagal menghapus data RTLH."); } finally { setIsSubmitting(false); }
    }
