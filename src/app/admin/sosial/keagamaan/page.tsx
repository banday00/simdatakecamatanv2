...
export default function KeagamaanPage() {
    ...
    const { data, isLoading, create, update, remove } = useCrud<Row>({ table: "social_religious" });
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch { alert("Gagal menyimpan data tempat ibadah."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await remove(deleteRow.id); setDeleteRow(null); }
        catch { alert("Gagal menghapus tempat ibadah."); } finally { setIsSubmitting(false); }
    }
