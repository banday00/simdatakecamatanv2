...
export default function PenggunaAdminPage() {
    ...
    const fetchUsers = useCallback(async () => {
        if (!tenant?.id || !isAuthorized) return;
        setIsLoading(true);
        try {
            ...
            setData((rows as UserRow[]) || []);
        } catch { /* silent */ } finally { setIsLoading(false); }
    }, [tenant?.id, isAuthorized]);
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            if (editRow) {
                /* Update profile via client (no auth change) */
                ...
                if (formData.password && String(formData.password).trim() !== "") {
                    const pwdRes = await fetch("/api/admin/users", {
                        method: "PATCH",
                    ...
                    if (!pwdRes.ok) throw new Error(pwdResult.error || `Gagal ganti password HTTP ${pwdRes.status}`);
                }
            } else {
                ...
                console.log("[handleSubmit] API response:", res.status, result);
    ...
    async function handleDelete() {
        ...
        setIsSubmitting(true);
        try {
            const res = await fetch(`/api/admin/users?userId=${deleteRow.id}`, { method: "DELETE" });
            const result = await res.json();
