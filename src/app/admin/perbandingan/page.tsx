...
export default function PerbandinganPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            // Group indicators by table to minimize queries
            const tableMap = new Map<string, typeof INDICATORS[number][]>();
            for (const ind of INDICATORS) {
                if (!selectedIndicators.includes(ind.key)) continue;
                const list = tableMap.get(ind.table) || [];
                list.push(ind);
                tableMap.set(ind.table, list);
            }

            // Fetch per table
            const results: Record<string, Record<string, number>> = {};
            for (const kelId of selectedKels) results[kelId] = {};

            for (const [table, inds] of tableMap) {
                const { data: rows } = await supabase
                    .schema("sidakota")
                    .from(table)
                    .select("*")
                    .eq("tenant_id", tenant.id)
                    .in("kelurahan_id", selectedKels);

                if (rows) {
                    for (const row of (rows as unknown as Record<string, unknown>[])) {
                        const kelId = String(row.kelurahan_id);
                        if (!results[kelId]) continue;
                        for (const ind of inds) {
                            // Take latest (first due to DESC order)
                            if (results[kelId][ind.key] == null && row[ind.col] != null) {
                                results[kelId][ind.key] = Number(row[ind.col]);
                            }
                        }
                    }
                }
            ...
            // Build flat data for table + charts
            const kelData: KelData[] = selectedKels.map((kelId) => {
                const kel = kelurahans.find((k) => k.id === kelId);
                return {
                    kelurahan_id: kelId,
                    kelurahan_nama: kel?.nama || "—",
                    ...results[kelId],
                };
            });
    ...
    }, [tenant, selectedKels, selectedIndicators, kelurahans]);
