...
export default function InsidenPage() {
    ...
    const { data, isLoading, create, update, remove } = useCrud<Row>({ table: "security_incidents" });
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch { alert("Gagal menyimpan rekam insiden."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await remove(deleteRow.id); setDeleteRow(null); }
        catch { alert("Gagal menghapus log kejadian."); } finally { setIsSubmitting(false); }
    }
