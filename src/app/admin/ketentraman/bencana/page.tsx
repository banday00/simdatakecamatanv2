...
const columns: Column<Row>[] = [
    ...
    },
    {
        ...
        render: (v) => <span className={`inline-flex px-2 py-0.5 text-xs font-bold rounded-full border ${riskColors[String(v)] || riskColors.Rendah}`}>{String(v)}</span>
    },
...
export default function BencanaPage() {
    ...
    const tinggi = data.filter((r) => r.tingkat_risiko === "Tinggi").length;
    const sedang = data.filter((r) => r.tingkat_risiko === "Sedang").length;
    const totalKK = data.reduce((s, r) => s + (r.jumlah_kk_terdampak || 0), 0);
    ...
    async function handleSubmit(fd: Record<string, unknown>) {
        ...
        catch { alert("Gagal menyimpan pemetaan bencana."); } finally { setIsSubmitting(false); }
    }

    async function handleDelete() {
        if (!deleteRow) return; setIsSubmitting(true);
        try { await remove(deleteRow.id); setDeleteRow(null); }
        catch { alert("Gagal menghapus data zona rawan."); } finally { setIsSubmitting(false); }
    }
