...
export default function BeritaPage() {
    ...
    const { data, isLoading, create, update, remove } = useCrud<BeritaRow>({ table: "news_articles" });
    ...
    async function handleSubmit(formData: Record<string, unknown>) {
        ...
        try {
            ...
            if (formData.gambar_file && tenant?.id) {
                const file = formData.gambar_file as File;
                const fileExt = file.name.split('.').pop();
                const fileName = `berita_${tenant.id}_${Date.now()}.${fileExt}`;
